...
// eslint-disable-next-line @typescript-eslint/ban-types
type DeepReadonly<T> = T extends Function | PrimitiveType
  ? T
...
export const createStateForge = <State, Actions>(rootPath: string) => {
  ...
  const createSlice = <T>(init: T, fn: (on: On<Actions>) => void): T => {
    console.warn("[createSlice]", init)
  ...
  const createStore = (slices: State, middleware = defaultMiddleware) => {
    ...
    // @TODO: adapter로 빼야함. React Adapter!
    // for React
